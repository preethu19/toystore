...
right_shoulder, rsport = 'C', 7777
left_shoulder, lsport = 'F', 9000
back, backport = 'G', 4444

right_thigh, rtport = 'N', 3333
...
joint_device = {'button': button, 'right_shoulder': right_shoulder, 'back': back, 'right_thigh': right_thigh, 'left_shoulder': left_shoulder}

device_list = [button, right_shoulder, back, right_thigh, left_shoulder]
port_list = [buttonport, rsport, backport, rtport, lsport]
...
sockets = {'button': socks[button], 'right_shoulder': socks[right_shoulder], 'back': socks[back], 'right_thigh': socks[right_thigh], 'left_shoulder': socks[left_shoulder]}
...
while True:  # time.time() - init <=10
    ...
    pose_detect = "pick|right|gravY={}|yaw={}|left|gravY={}|yaw={}".format(float(d[right_shoulder][gravaccy]), (float(d[right_shoulder][calcYaw]) - float(d[back][calcYaw])), float(d[left_shoulder][gravaccy]), (float(d[left_shoulder][calcYaw]) - float(d[back][calcYaw])))
    ...
    if (-8 >= float(d[right_shoulder][gravaccy])) or ( -8 >= float(d[left_shoulder][gravaccy])):
        # print("Place")
        placeState = 'place'
        # sockUDP.sendto("place".encode(), (UDP_IP, UDP_PORT))
    else:
        placeState = 'no place'
    # elif (( -8 < float(d[right_shoulder][gravaccy])) and (-8 < float(d[left_shoulder][gravaccy]))):
    #     placeState = 'no place'
        # print("Sending pick")
        # sockUDP.sendto(pose_detect.encode(), (UDP_IP, UDP_PORT))


    lastgravYRhand = float(d[right_shoulder][gravaccy])
    lastgravYLhand = float(d[left_shoulder][gravaccy])
